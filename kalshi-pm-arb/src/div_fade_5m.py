...
def maybe_log_5m_signal(
    ...
    if _should_trade_live(asset, signal) and token_id:
        if DIV_FADE_EXECUTOR_DAEMON:
            # Execution handed off to div_fade_executor.py daemon.
            # That process applies entry delay, OBI gate, and maker orders.
            log.info(
                "[DIV_FADE_5M] Signal logged — execution deferred to div_fade_executor daemon "
                "(%s %s pm=%.1f¢)",
                asset, signal, pm_price,
            )
        else:
            # ── Legacy inline FAK execution ───────────────────────────────
